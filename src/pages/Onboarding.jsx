...
export default function Onboarding() {
  ...
  return (
    <div style={{ background: '#16161A', color: '#fff', minHeight: '100vh', fontFamily: 'Inter, sans-serif', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Top bar */}
      <div style={{ padding: '16px 40px', borderBottom: '1px solid #1a1a1a', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#1C1C21' }}>
        <img src={logo} alt="Phaero" style={{ height: '44px', cursor: 'pointer' }} onClick={() => navigate('/')} />
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: '#202024', border: '1px solid #1e1e1e', borderRadius: '20px', padding: '6px 14px' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 24px' }}>
        ...
        <div style={{ width: '100%', maxWidth: '720px', marginBottom: '20px', animation: 'fadeUp 0.7s ease forwards', position: 'relative' }}>
          ...
          <textarea
            ...
            rows={5}
            style={{ width: '100%', background: '#202024', border: '1px solid #2a2a2a', borderRadius: '12px', padding: '20px 20px 48px', color: '#fff', fontSize: '15px', outline: 'none', resize: 'none', fontFamily: 'Inter, sans-serif', lineHeight: '1.7', boxSizing: 'border-box', position: 'relative', zIndex: 1, transition: 'border-color 0.2s, box-shadow 0.2s' }}
        ...
        <div style={{ width: '100%', maxWidth: '720px', marginBottom: '36px', animation: 'fadeUp 0.8s ease forwards' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
            {chips.map((chip, i) => {
              ...
              return (
                <button key={i} onClick={() => handleChip(chip)} style={{ background: active ? '#D4AF37' : '#202024', color: active ? '#000' : '#555', border: `1px solid ${active ? '#D4AF37' : '#222'}`, padding: '8px 14px', borderRadius: '20px', fontSize: '13px', fontWeight: '600', cursor: 'pointer', transition: 'all 0.2s', display: 'flex', alignItems: 'center', gap: '6px' }}>
      ...
      {showInsufficientCredits && (
        <div onClick={() => setShowInsufficientCredits(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', backdropFilter: 'blur(8px)' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#202024', border: '1px solid #1e1e1e', borderRadius: '16px', maxWidth: '380px', width: '100%', padding: '32px', textAlign: 'center' }}>
